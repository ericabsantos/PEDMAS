...
To analyze education’s relationship on wages, we will create new variables:
...
```{r}
#Run Regression
model <- lm(wage~college_graduate,data=wageData)
...
```{r}
#Regression with college graduate status and age
model2 <- lm(wage ~ college_graduate + age, data = wageData)
...
1.13554 Increase in question 2. The college graduates tend to be older and that age is positively related to wages. 
...
model3 <- lm(wage ~ edu_somecollege + edu_hsgrad + edu_advdegree + age, data = wageData)

summary(model3)

```

The data in 1.1 was raw, unadjusted for factors such as age. This effects the mean data, as there are unaccounted for age differences through out the groups of education levels. 
